...
const bruningEffect = newEffect(40, e => {
...
const bruningStatusEffect = new StatusEffect("forcebruning");

bruningStatusEffect.speedMultiplier = 1;
bruningStatusEffect.damage  = 1;
bruningStatusEffect.effect = bruningEffect;

const blockType = extendContent(Block, "未来立场Y", {
    _timerUse: 0,
            ...
            Vars.unitGroup.intersect(tile.drawx() - realRadius, tile.drawy() - realRadius, realRadius * 2, realRadius * 2, new Cons({
                get(v) {
                    if (v.getTeam() != tile.getTeam() && Intersector.isInsideHexagon(v.getX(), v.getY(), realRadius * 2, tile.drawx(), tile.drawy())) {
                        v.applyEffect(bruningStatusEffect, 30);
                    }
                },
            }));
        }
