...
const wetEffect = newEffect(40, e => {
...
const wetStatusEffect = new StatusEffect("forcewet");

wetStatusEffect.damageMultiplier  = 0.4;
wetStatusEffect.effect = wetEffect;

const blockType = extendContent(Block, "未来立场Z", {
    _timerUse: 0,
            ...
            Vars.unitGroup.intersect(tile.drawx() - realRadius, tile.drawy() - realRadius, realRadius * 2, realRadius * 2, new Cons({
                get(v) {
                    if (v.getTeam() != tile.getTeam() && Intersector.isInsideHexagon(v.getX(), v.getY(), realRadius * 2, tile.drawx(), tile.drawy())) {
                        v.applyEffect(wetStatusEffect, 30);
                    }
                },
            }));
        }
