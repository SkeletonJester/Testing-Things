...
// 正向激光
Effects.effect(pointingLaserEffect, Color.cyan,
b.data.getSourceX(),
b.data.getSourceY(),
0,
b.data
...
// 喷个火球
Call.createBullet(Bullets.cryoShot, b.getTeam(), targetX, targetY, Mathf.random(360), 1, 1);
// 子弹瞬移！啪嚓就撞上
b.x = targetX;
b.y = targetY;
b.data.hit = true;
...
/** 指向性激光炮塔 */
const pointingLaserTurret = extendContent(PowerTurret, "超级光棱塔", {
chargeTime: 60,
cooldownDelay: 60,
...
// -=-=-常用属性 -=-=- 
 // -= 子弹 =- 
 // 直接伤害 
 pointingLaserBulletType.damage = 450; 
 // 爆炸伤害 
 pointingLaserBulletType.splashDamage = 30; 
 // 爆炸范围（一格 = 8 似乎，40 = 5 格） 
 pointingLaserBulletType.splashDamageRadius = 20; 
 // 发射效果 
 ...
 pointingLaserBulletType.fragBullet = Bullets.lightning
; 
 pointingLaserBulletType.fragBullets = 4; 
 // -= 炮塔 =- 
 // 发射声音 
